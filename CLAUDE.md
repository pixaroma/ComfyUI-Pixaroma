...
25. **Block-level `execCommand("insertHTML")` drops staged foreColor/hiliteColor — call `_restageColors()` after every block insert** — inserting a `<table>`, `<pre>`, or `<hr>` splits the caret out of its current inline formatting context. Any color the user had staged (picked-but-not-yet-typed) is silently dropped. Without compensation: user picks orange, inserts a grid, types in a cell → white text. `insertAtSavedRange()` in `blocks.mjs` calls `editor._restageColors?.()` after the `execCommand` so the picked colors from `_textColorBtn` / `_hiColorBtn` `--pix-note-tbtn-tint` are replayed against the caret's NEW position. Ordering matches Pattern #21: apply hiliteColor FIRST, foreColor SECOND (hiliteColor on collapsed clears staged foreColor). The two color picker buttons are exposed on the editor instance (`this._textColorBtn`, `this._hiColorBtn`) so any future block-insert path can call the helper. Any NEW block-insert path must use `insertAtSavedRange` OR call `_restageColors()` explicitly after DOM mutation — otherwise the same class of bug recurs.
