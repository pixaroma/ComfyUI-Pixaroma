...
| Change Remove Background Pixaroma behavior / outputs | `nodes/node_remove_background.py` (single file, ~180 lines, Python-only, no JS). Built-in dropdown reads files from `ComfyUI/models/background_removal/` via `folder_paths.get_filename_list`; empty folder shows a sentinel option that raises `_INSTALL_MESSAGE` (with download URLs) on Run. Filename -> resolution rule in `_resolution_for_filename`: stem contains `matt` -> 2048; `hr` matched word-piece (regex `(?<![a-z])hr(?![a-z])`, case-insensitive) -> 2048; else 1024. ComfyUI's native loader hardcodes 1024 in `comfy/background_removal/birefnet.json` so we bypass `comfy.bg_removal_model.load()` and construct a `_PixaromaBgModel` subclass (mirrors `BackgroundRemovalModel.__init__` line-for-line but takes a config dict instead of a json path) with our chosen `image_size`. Sanity-checks state dict for the BiRefNet backbone marker `bb.layers.1.blocks.0.attn.relative_position_index` and refuses non-BiRefNet weights with a clear error. Module-level `_MODEL_CACHE` is an `OrderedDict` LRU cap 2, keyed by `(absolute_path, image_size)` so a rename / resolution swap correctly cache-busts. Calls `bg_model.encode_image(image)` (same call native RemoveBackground makes - see ComfyUI's `comfy/bg_removal_model.py` `encode_image` for the BiRefNet sigmoid pipeline). Normalizes mask to canonical `(B, H, W)` shape from the wrapper's `(B, 1, H, W)` output. Builds RGBA via `torch.cat([image[..., :3], mask.unsqueeze(-1)], dim=-1)`. Three outputs: `image` (RGBA), `mask` (fg=1), `inverted_mask` (bg=1). The old `BACKGROUND_REMOVAL` wire input was removed in 1.3.33 - hard break, no migration shim; users with old workflows just pick a model from the new dropdown. |
