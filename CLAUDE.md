...
| Depth Map Pixaroma — depth estimation + invert/contrast/blur with inline preview | `nodes/node_depth_map.py`. Standalone node that owns ALL depth-model logic — Audio Depth Pixaroma now consumes its output as an `IMAGE` input. Depth model dispatched via `_get_depth_predictor` (process-wide `_DEPTH_CACHE` keyed by model name + device). Two backends: (1) MiDaS via `_build_midas_predictor` → torch.hub `intel-isl/MiDaS` with `_safe_hub_load` shim that hides `comfyui_controlnet_aux`'s bundled midas package; failure -> `_midas_install_hint` (weight URL + checkpoints path + repo-zip fallback). (2) Depth Anything V2 via `_build_da2_predictor` → `transformers.pipeline("depth-estimation", model=…)` (lazy import); failure -> `_da2_install_hint` (`pip install transformers` + HF cache dir note). Post-processing: `depth_invert` → `depth_contrast` power curve → `depth_blur` (separable Gaussian via `_gaussian_blur_2d`). Output: `depth_map` IMAGE [1, H, W, 3] with R=G=B=depth, normalized to [0, 1]. `OUTPUT_NODE = True` and the return dict carries `{"ui": {"images": [...]}}` so the rendered depth map shows inline in the node body (Preview Image Pixaroma pattern, PNG written to `temp/`). Caching at the node level means downstream tweaks (audio params on the next node) don't trigger re-running this node — depth pass is paid once per (image, model, contrast, invert, blur) combination. |
