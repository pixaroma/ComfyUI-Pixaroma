...
15. **Custom widgets need `options: { canvasOnly: true }` or they render in the Parameters tab and corrupt node-body layout.** The Vue frontend's right-sidebar Parameters tab iterates `node.widgets` and draws each one in the panel column. For our canvas-painted internal widgets (`pixaroma_buttons`, `pixaroma_strip` in `js/preview/index.js`) this caused two compounding bugs: (a) the user saw `pixaroma_buttons` and `pixaroma_strip` rows in the Parameters tab with the buttons + thumbnail strip duplicated there (visually wrong, they're not user inputs); (b) each Parameters-panel `draw()` call wrote stale `node._pixaromaCells` with Parameters-panel coords, then the node body's draw read the same cache and reflowed the grid into a 1xN row at Parameters-panel width. The fix is the bundle's standard exclusion flag — `widget.options.canvasOnly = true`. Filter found in the bundle: `t.filter(e => !(e.options?.canvasOnly || e.options?.hidden || e.options?.advanced && !n.value))`. Apply this to ANY new internal canvas-only custom widget; `serialize: false` alone is not enough (it only stops the value being saved into the workflow JSON, not the panel rendering).
