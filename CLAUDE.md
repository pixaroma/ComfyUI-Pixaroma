...
| Audio Depth Pixaroma — change widgets / aspect ratios / parallax math | `nodes/node_audio_depth.py`. MiDaS load goes through `_get_midas` (process-wide cache keyed by model name + device); failure path emits a manual-install message via `_midas_install_hint` (weight URL + target path under `<models_dir>/torch_hub/checkpoints/`, plus repo-zip fallback). Uses `comfy.utils.ProgressBar` for the per-frame loop. Motion modes (`radial`/`horizontal`/`vertical`/`combined`) dispatched inside the per-frame loop; sway/bob use `sin`/`cos` of `t * 2π * 0.25 Hz` so the camera completes one cycle every 4 seconds. Depth post-processing: `depth_invert` then `depth_contrast` power-curve. Audio reactivity: `_AUDIO_BANDS_HZ` table + FFT bandpass via `_bandpass_fft` applied BEFORE RMS so per-band envelope feels right. `loop_safe` ramps the envelope to zero across the first/last `0.5s * fps` frames. Outputs include a `depth_map` IMAGE (single-frame, 3-channel by expand) for inspection. No JS frontend, no server route. |
