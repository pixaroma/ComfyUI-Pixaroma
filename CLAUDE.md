...
| Audio React Pixaroma — change motion mode or overlay effect | `nodes/node_audio_react.py` (single file). Motion modes are `_motion_*` methods (`scale_pulse`, `zoom_punch`, `shake`, `ripple`, `slit_scan`, `kaleidoscope`); overlays are `_overlay_*` methods (`glitch`, `bloom`, `vignette`, `hue_shift`). Helpers `_bandpass_fft`, `_audio_envelope`, `_onset_track`, `_process_aspect` are duplicated locally from audio_depth (no cross-file import). Per-frame loop in `generate()` dispatches motion mode → `F.grid_sample` warp → overlay chain (each overlay short-circuits when strength == 0) → center-crop → stack. Pairs with audio_depth as the non-depth alternative. No JS, no server route. |
