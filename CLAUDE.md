...
20. **Text + highlight picker icons mirror the cursor's effective inline colour, gated by a 1s post-pick suppress window** — `_mirrorPickerColors` in `js/note/toolbar.mjs` runs from the `selectionchange` handler, walks up from the caret to the nearest inline `style.color` (text picker) and `style.backgroundColor` (highlight picker), normalises via `colorToHex`, and writes `--pix-note-tbtn-tint`. None found → `removeProperty` so the icon falls back to currentColor. Skipped when the selection is non-collapsed (range may span mixed colours, no single right answer). The suppress window — `_suppressMirrorUntil = Date.now() + 1000` set inside both `onPick` callbacks — is the fix for the historical clobber bug: `execCommand("foreColor"/"hiliteColor")` on a collapsed selection STAGES without mutating the DOM, so a mirror running immediately after a pick would read the cursor's pre-pick parent colour and overwrite the just-picked tint. The 1s window gives the user time to start typing — the typed text gets wrapped in a span with the picked colour, and subsequent mirror runs read the picked colour from the new span. The Clear / null path of each picker MUST `removeProperty("--pix-note-tbtn-tint")` (not just strip the inline colour), otherwise the icon stays tinted while the cursor is in newly-uncoloured text. Bg / Btn / Ln pickers stay on the "last explicit pick" model — their state is in `cfg.*Color`, not in the cursor's context, so there's nothing meaningful to mirror.

21. **Chrome `hiliteColor` on collapsed selection clears staged `foreColor`, AND occasionally misplaces the new span at editArea start (caret-jump bug) — highlight picker uses direct DOM `range.insertNode` for collapsed cursor.** `execCommand("hiliteColor", c)` on a collapsed selection CREATES a `<span style="background-color:...">` in the DOM at the cursor AND clears any previously-staged foreColor. Two consequences: (a) user picks text orange → picks highlight green → types → gets WHITE text on green bg (not orange on green); (b) for some span-nesting configurations the new span lands at the START of editArea instead of at the cursor, visible to the user as the caret jumping to the beginning of the note. Fix in [js/note/toolbar.mjs](js/note/toolbar.mjs)'s highlight onPick: for COLLAPSED cursor build the `<span style="background-color:...">` directly with `document.createElement` + `range.insertNode`, then place the caret inside via `range.selectNodeContents(span); range.collapse(true)`. For NON-collapsed range, keep `execCommand("hiliteColor", c)` since it correctly handles selections that span multiple inline elements. The Pattern-21 (a) compensation still applies in BOTH paths: after the bg apply, read `textColorBtn.style.getPropertyValue("--pix-note-tbtn-tint")` and replay `execCommand("foreColor", thatColor)` to re-stage. The reverse direction (foreColor first, no hilite) doesn't need compensation — foreColor on collapsed stages without DOM mutation and doesn't clear hiliteColor staging. Reset / transparent on collapsed cursor strips the bg from the cursor's nearest containing inline-styled element via a walk-up; on a range it strips bg from every intersecting element (mirror of the text-color Clear path).
