...
9. **Do NOT call `node.removeInput(idx)` to hide auto-created input slots for hidden STRING widgets** — The Vue frontend auto-exposes primitive-type required inputs (STRING/INT/FLOAT) as convertible input slots that flash a grey dot on hover. Calling `removeInput` to suppress that dot causes the saved workflow JSON to have `"inputs": []`, and on workflow RELOAD Vue fails to reconnect the saved `widgets_values[0]` (the JSON string) to the hidden STRING widget — the widget deserializes to its Python default. The whole hidden-widget state pattern silently reverts on every workflow open. The correct approach is to leave the input slot intact and accept the hover dot, OR use a different architectural approach (store state in `node.properties` + Python `"hidden"` inputs + a `graphToPrompt` hook) if the dot is unacceptable. Resolution Pixaroma accepts the dot; so does Note Pixaroma.
