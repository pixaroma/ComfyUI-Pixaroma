...
| Audio Depth Pixaroma — change widgets / aspect ratios / parallax math | `nodes/node_audio_depth.py`. MiDaS load goes through `_get_midas` (process-wide cache keyed by model name + device); failure path emits a manual-install message via `_midas_install_hint` (weight URL + target path under `<models_dir>/torch_hub/checkpoints/`, plus repo-zip fallback). Uses `comfy.utils.ProgressBar` for the per-frame loop. Motion modes (`radial`/`horizontal`/`vertical`/`combined`) dispatched inside the per-frame loop; sway/bob use `sin`/`cos` of `t * 2π * motion_speed` (default 0.2 Hz = 5 s/cycle). Depth post-processing: `depth_invert` then `depth_contrast` power-curve then `depth_blur` (separable Gaussian via `_gaussian_blur_2d`). Audio reactivity: `_AUDIO_BANDS_HZ` table + FFT bandpass via `_bandpass_fft` applied BEFORE RMS; the moving-average smoothing kernel size is exposed as the `smoothing` widget. `loop_safe` ramps the envelope to zero across the first/last `0.5s * fps` frames. `base_motion` linearly blends an always-on motion floor onto the envelope. `camera_shake` adds depth-independent grid offsets via random-anchor (1/sec) + cosine-interpolated drift. Outputs include a `depth_map` IMAGE (single-frame, 3-channel by expand) for inspection. Zero-length-audio guard raises ValueError up front. No JS frontend, no server route. |
| Save Mp4 Pixaroma — change widgets / encoder flags / output naming | `nodes/node_save_mp4.py`. ffmpeg binary resolved via `_resolve_ffmpeg` (imageio-ffmpeg first, ffmpeg on PATH fallback, clear install hint on failure). Frames piped to ffmpeg's stdin as raw rgb24 (no temp PNGs). Audio (optional) is written to a temp WAV via `_write_wav_pcm16` (stdlib `wave` + numpy, NO torchaudio dep) and passed as a second `-i` input so muxing is one ffmpeg call. `pingpong` is built in Python by concatenating the reversed-tail batch (excluding both endpoints to avoid duplicate-frame stalls). `trim_to_audio` adds `-shortest` only when audio is present. Output naming via `folder_paths.get_save_image_path` so it auto-increments and respects ComfyUI subfolder conventions. `OUTPUT_NODE = True` (terminal). Returns `{"ui": {"images": [{...}]}}` so the saved file appears in the workflow output panel. No JS frontend, no server route. |
