...
3. **`_audio_envelope`, `_bandpass_fft`, `_onset_track`, `_process_aspect` are duplicated locally from audio_depth — no cross-file import** — the two nodes are intentionally independent. If you fix a bug in audio_depth's envelope, manually port to audio_react and vice versa. Cross-file imports would make audio_react fail to load when audio_depth is disabled.

4. **Print line uses ASCII `->`, not `→` (U+2192)** — Windows console default codec (cp1252) can't encode the arrow. Crashes the generate() call before frame 1. The audio_depth print intentionally avoids non-ASCII for the same reason.
...
| Depth Map Pixaroma — depth estimation + invert/contrast/blur with inline preview | `nodes/node_depth_map.py`. Standalone DA2-only node (Small / Base / Large) — Audio Depth Pixaroma consumes the `depth_map` IMAGE output. Model files live PORTABLY under `<ComfyUI>/models/depth-anything-v2/<Size>/` (NOT in `~/.cache/huggingface/`) so moving the ComfyUI install moves the weights with it. `_ensure_da2_local` checks for the three required files (`config.json`, `preprocessor_config.json`, `model.safetensors`); missing → `huggingface_hub.snapshot_download(repo_id, local_dir, allow_patterns=…)` into the same folder; auto-download failure → `_da2_install_hint` prints the exact target folder + three direct `https://huggingface.co/<repo>/resolve/main/<file>` URLs for manual install. Loaded via `AutoImageProcessor.from_pretrained(local_dir)` + `AutoModelForDepthEstimation.from_pretrained(local_dir)`. Inference: `processor(images=pil)` → `model(**inputs).predicted_depth`. Predictor cached process-wide in `_DEPTH_CACHE` keyed by model name + device. Outputs: `(image, depth_map)` — `image` is a passthrough so Audio Depth can wire from one source. Post-processing: `depth_invert` → `depth_contrast` power curve → `depth_blur` (separable Gaussian via `_gaussian_blur_2d`). Inline preview via `OUTPUT_NODE = True` + `{"ui": {"images": [...]}}` (PNG written to `temp/`). |
| Audio React Pixaroma — change motion mode or overlay effect | `nodes/node_audio_react.py` (single file). Motion modes are `_motion_*` methods (`scale_pulse`, `zoom_punch`, `shake`, `drift`, `rotate_pulse`, `ripple`, `swirl`, `slit_scan`); overlays are `_overlay_*` methods (`glitch`, `bloom`, `vignette`, `hue_shift`). Helpers `_bandpass_fft`, `_audio_envelope`, `_onset_track`, `_process_aspect` are duplicated locally from audio_depth (no cross-file import). Per-frame loop in `generate()` dispatches motion mode → `F.grid_sample` warp → overlay chain (each overlay short-circuits when strength == 0) → stack. Pairs with audio_depth as the non-depth alternative. No JS, no server route. |
| Audio Depth Pixaroma — change widgets / parallax math | `nodes/node_audio_depth.py`. Depth model logic LIVES IN `node_depth_map.py` now — this node receives a `depth_map` IMAGE input (required) and uses it directly. Body converts the input via `depth_2d = depth_map[0].mean(dim=-1)` (averages RGB channels so colored hand-edits work), resizes to render dims with bicubic F.interpolate, and renormalizes to [0, 1] (so user-edited maps with arbitrary value ranges still work). Uses `comfy.utils.ProgressBar` for the per-frame loop. Motion modes (`radial`/`horizontal`/`vertical`/`combined`) dispatched inside the per-frame loop; sway/bob use `sin`/`cos` of `t * 2π * motion_speed` (default 0.2 Hz = 5 s/cycle). Audio reactivity: `_AUDIO_BANDS_HZ` table + FFT bandpass via `_bandpass_fft` applied BEFORE RMS; moving-average smoothing kernel size exposed as the `smoothing` widget. `loop_safe` ramps the envelope to zero across the first/last `0.5s * fps` frames AND the same ramp is applied to camera_shake so the loop closes cleanly. `base_motion` linearly blends an always-on motion floor onto the envelope (applied BEFORE loop_safe so the ramp can pull it to true zero). `camera_shake` adds depth-independent grid offsets via random-anchor (1/sec) + cosine-interpolated drift. `edge_headroom` renders at `base_size * headroom` (snapped to mult-of-8) and center-crops back to base_size after warp so motion has a safety zone outside the visible frame. `padding_mode='border'` in `grid_sample` (NOT `reflection`) so off-frame areas smear the edge pixel instead of mirroring the silhouette. Outputs: `video_frames`, `audio` (passthrough), `fps`. Zero-length-audio guard raises ValueError up front. No JS frontend, no server route. |
