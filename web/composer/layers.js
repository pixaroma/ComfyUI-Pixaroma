export class PixaromaLayers {
    static getTransformedPoints(layer) {
        ...
        const ptsIdentity = [
            ...
            {x:-w/2, y:0}, {x:w/2, y:0}, {x:0, y:-h/2}, {x:0, y:h/2}, {x:0, y:-h/2 - 30}
        ];
    ...
    static isPointInLayer(px, py, layer) {
        ...
        const h = layer.img.height * layer.scaleY;
        const rad = layer.rotation * Math.PI / 180;
        const dx = px - layer.cx; const dy = py - layer.cy;
    ...
    static fitLayerToCanvas(layer, docWidth, docHeight, mode="width") {
        ...
        let w = layer.img.width; let h = layer.img.height;
        ...
        layer.cx = docWidth / 2; layer.cy = docHeight / 2;
    }
