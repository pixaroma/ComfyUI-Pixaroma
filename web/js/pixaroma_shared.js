...
// ═══════════════════════════════════════════════════════════════
// Shared Node Preview System
// All Pixaroma nodes use this for consistent preview display.
// Change once here → all nodes update.
// ═══════════════════════════════════════════════════════════════

/**
 * Creates the DOM elements for a node preview area.
 * Returns an object with all the parts so the node can wire them up.
 *
 * @param {string} titleText     – e.g. "Paint", "3D Builder"
 * @param {string} subtitleText  – e.g. "Pixaroma"
 * @param {string} instructionText – e.g. "Click 'Open Paint' to start"
 * @returns {{ container, preview, dummy, infoLabel }}
 */
export function createNodePreview(titleText, subtitleText, instructionText) {
  // ── Outer container (the DOM widget root) ──
  const container = document.createElement("div");
  container.style.cssText = `
    display: none;
    flex-direction: column;
  ...
  // ── Preview wrapper: enforced square via padding-bottom trick ──
  // padding-bottom % is always relative to the element's WIDTH,
  // so padding-bottom:100% + height:0 = perfect square in every
  // layout mode (old nodes, Nodes 2.0, any browser).
  ...
  // ── Preview image: centered inside the square box ──
...
/**
 * Show a preview image (from dataURL or url) in the shared preview area.
 * Hides the placeholder, shows the letterboxed image + dimensions.
 *
 * @param {{ preview, dummy, infoLabel }} parts
 * @param {string} src       – image src (dataURL or URL)
 * @param {string} dimText   – dimension text e.g. "1024×768"
 * @param {object} node      – the LiteGraph node (for setDirtyCanvas)
 */
export function showNodePreview(parts, src, dimText, node) {
  ...
  img.onload = () => {
    ...
    // JS fallback for old ComfyUI where padding-bottom trick doesn't work:
    // poll for a few frames until the element has a measured width, then
    // force height = width to make the box square.
...
// Force an element's height to equal its width (square).
// Polls via requestAnimationFrame for up to ~1 second to catch late layout.
...
/**
 * Try to restore a preview from a saved JSON string containing composite_path.
 * Used when a workflow is loaded and the node needs to show its last preview.
 *
 * @param {{ preview, dummy, infoLabel }} parts
 * @param {string} json  – the raw JSON string from the widget value
...
/**
 * Activate the preview container after a short delay (avoids flicker).
 *
 * @param {{ container }} parts
 * @param {object} node
 */
export function activateNodePreview(parts, node) {
  // Hook node resize so the preview box stays square when
  // the user manually drags the node wider/narrower.
