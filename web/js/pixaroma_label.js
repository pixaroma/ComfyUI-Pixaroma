...
const TEXT_SWATCHES = [
    ...
    "#f66744", "#cc3333", "#33aa33", "#3388dd", "#ddaa00",
];
const BG_SWATCHES = [
    "#333333", "#444444", "#555555", "#222222", "#111111",
    "#f66744", "#593930", "#355735", "#354f6b", "#4f3560",
    "#ffe0e0", "#e0ffe0", "#e0e0ff", "#fff3e0", "#f0f0f0",
];
...
function parseCfg(node) {
    ...
    try { return { ...DEFAULTS, ...JSON.parse(w.value) }; }
...
class LabelEditor {
    ...
    _build() {
        ...
        const lbl = (text) => { const l = el("div", "pix-lbl-lbl"); l.textContent = text; return l; };
        ...
        ta.value = c.text; ta.rows = 3;
        ta.addEventListener("input", () => { c.text = ta.value; this._updatePreview(); });
        ...
        sizeRange.type = "range"; sizeRange.min = 8; sizeRange.max = 64; sizeRange.value = c.fontSize;
        ...
        tcPicker.type = "color"; tcPicker.value = c.fontColor;
        const tcHex = document.createElement("input");
        tcHex.type = "text"; tcHex.className = "pix-lbl-hex"; tcHex.value = c.fontColor;
        tcPicker.addEventListener("input", () => {
            c.fontColor = tcPicker.value; tcHex.value = tcPicker.value;
        ...
        tcHex.addEventListener("input", () => {
            ...
            if (/^#[0-9a-fA-F]{6}$/.test(v)) {
                c.fontColor = v; tcPicker.value = v;
        ...
        transpSw.onclick = () => {
            ...
            bgPicker.disabled = true; bgHex.disabled = true;
            bgSwatches.querySelectorAll(".pix-lbl-swatch,.pix-lbl-swatch-transp").forEach(s => s.classList.remove("active"));
        ...
        this._buildSwatches(bgSwatches, BG_SWATCHES, c.backgroundColor, (color, swEls) => {
            ...
            bgPicker.value = color; bgPicker.disabled = false;
            bgHex.value = color; bgHex.disabled = false;
        ...
        bgHex.type = "text"; bgHex.className = "pix-lbl-hex";
        ...
        bgPicker.addEventListener("input", () => {
            c.backgroundColor = bgPicker.value; bgHex.value = bgPicker.value;
            transpSw.classList.remove("active");
        ...
        bgHex.addEventListener("input", () => {
            ...
            if (/^#[0-9a-fA-F]{6}$/.test(v)) {
                c.backgroundColor = v; bgPicker.value = v;
                transpSw.classList.remove("active");
        ...
        row4.appendChild(this._rangeField("Padding", c.padding, 0, 60, 1, v => { c.padding = v; }));
        row4.appendChild(this._rangeField("Radius", c.borderRadius, 0, 40, 1, v => { c.borderRadius = v; }));
        body.appendChild(row4);

        const row5 = el("div", "pix-lbl-row");
        row5.style.marginBottom = "0";
        row5.appendChild(this._rangeField("Opacity", c.opacity, 0, 1, 0.05, v => { c.opacity = v; }));
        row5.appendChild(this._rangeField("Line Height", c.lineHeight, 1, 3, 0.1, v => { c.lineHeight = v; }));
    ...
    _rangeField(label, val, min, max, step, onChange) {
        ...
        l.className = "pix-lbl-lbl"; l.textContent = label;
        field.appendChild(l);
