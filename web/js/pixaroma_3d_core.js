...
function injectStyles() {
  ...
  for (const old of ["pixaroma-3d-styles", "pixaroma-3d-styles-v3", "pixaroma-3d-v4", "pixaroma-3d-v5"])
...
// ─── Editor ──────────────────────────────────────────────────
export class Pixaroma3DEditor extends PixaromaEditorBase {
  constructor() {
    super();
    this.docW = 1024;
    this.docH = 1024;
  ...
  _editorTitle() {
    return `3D Builder <span class="pxb-brand">Pixaroma</span>`;
  }

  _buildWorkspace() {
    const vp = document.createElement("div");
    vp.className = "p3d-viewport";
    this.el.viewport = vp;
    // Background image container (behind Three.js canvas)
    ...
    ti.className = "p3d-tool-info";
    ti.textContent = "Move (M) \u2014 Drag arrows to move object";
    this.el.toolInfo = ti;
    vp.appendChild(ti);
  ...
  _buildLeftSidebar() {
    ...
    left.className = "p3d-left";

    // Canvas Settings
    const cs = this._section("Canvas Settings");
    const ratioSel = document.createElement("select");
    ratioSel.className = "p3d-input";
    ratioSel.style.marginBottom = "5px";
    ...
    const wIn = this._numInput(this.docW);
    const hIn = this._numInput(this.docH);
    this.el.wIn = wIn;
    this.el.hIn = hIn;
    cs.appendChild(this._row("Width", wIn));
    cs.appendChild(this._row("Height", hIn));
    ...
    wIn.addEventListener("change", () => {
      this.docW = Math.max(64, parseInt(wIn.value) || 1280);
      wIn.value = this.docW;
      if (this._ratioLocked && this._activeRatio) {
        const [rw, rh] = this._activeRatio.v.split(":").map(Number);
        this.docH = Math.round((this.docW * rh) / rw);
        hIn.value = this.docH;
      }
      this._updateFrame();
    });
    hIn.addEventListener("change", () => {
      this.docH = Math.max(64, parseInt(hIn.value) || 720);
      hIn.value = this.docH;
      if (this._ratioLocked && this._activeRatio) {
        const [rw, rh] = this._activeRatio.v.split(":").map(Number);
        this.docW = Math.round((this.docH * rw) / rh);
        wIn.value = this.docW;
      }
      this._updateFrame();
    });
    ...
    bgIn.type = "color";
    bgIn.value = this.bgColor;
    bgIn.style.cssText =
      "width:50px;height:22px;cursor:pointer;border:1px solid #3a3d40;border-radius:4px;background:#111;";
    bgIn.addEventListener("input", () => {
      this.bgColor = bgIn.value;
      if (this.scene && !this.el.bgImgEl) this.scene.background = new THREE.Color(bgIn.value);
    });
    this.el.bgColor = bgIn;
    cs.appendChild(this._row("BG", bgIn));

    // Background Image (inside Canvas Settings)
    const bgTitle = document.createElement("div");
    bgTitle.style.cssText = "font-size:9px;color:#888;margin-top:6px;margin-bottom:3px;";
    bgTitle.textContent = "Background Image";
    cs.appendChild(bgTitle);
    const bgBtnRow = document.createElement("div");
    bgBtnRow.style.cssText = "display:flex;gap:5px;margin-bottom:6px;";
    const loadBgBtn = this._mkBtn("Load", () => this._loadBgImage(), "p3d-btn", "Load a background image");
    const removeBgBtn = this._mkBtn(
      "Remove",
      () => this._removeBgImage(),
      "p3d-btn p3d-btn-danger",
      "Remove background image"
    );
    loadBgBtn.style.flex = "1";
    removeBgBtn.style.flex = "1";
    bgBtnRow.append(loadBgBtn, removeBgBtn);
    cs.appendChild(bgBtnRow);
    const fitRow = document.createElement("div");
    fitRow.style.cssText = "display:flex;gap:5px;margin-bottom:6px;";
    const fitWBtn = this._mkBtn("Fit Width", () => this._fitBg("width"), "p3d-btn", "Fit to canvas width (keep ratio)");
    const fitHBtn = this._mkBtn(
      "Fit Height",
      () => this._fitBg("height"),
      "p3d-btn",
      "Fit to canvas height (keep ratio)"
    );
    const fitFillBtn = this._mkBtn("Fill", () => this._fitBg("fill"), "p3d-btn", "Fill canvas (cover, keep ratio)");
    fitWBtn.style.flex = "1";
    fitHBtn.style.flex = "1";
    fitFillBtn.style.flex = "1";
    fitRow.append(fitWBtn, fitHBtn, fitFillBtn);
    cs.appendChild(fitRow);
    const bgXR = this._sliderRow("X", -100, 100, 0, (v) => {
      this._bgImg.x = v;
      this._updateBgCSS();
    });
    const bgYR = this._sliderRow("Y", -100, 100, 0, (v) => {
      this._bgImg.y = v;
      this._updateBgCSS();
    });
    const bgScR = this._sliderRow("Scale", 1, 300, 100, (v) => {
      this._bgImg.scale = v;
      this._updateBgCSS();
    });
    const bgRotR = this._sliderRow("Rotate", -180, 180, 0, (v) => {
      this._bgImg.rotation = v;
      this._updateBgCSS();
    });
    const bgOpR = this._sliderRow("Opacity", 0, 100, 100, (v) => {
      this._bgImg.opacity = v;
      this._updateBgCSS();
    });
    this.el.bgX = bgXR.slider;
    this.el.bgXV = bgXR.val;
    this.el.bgY = bgYR.slider;
    this.el.bgYV = bgYR.val;
    this.el.bgSc = bgScR.slider;
    this.el.bgScV = bgScR.val;
    this.el.bgRot = bgRotR.slider;
    this.el.bgRotV = bgRotR.val;
    this.el.bgOp = bgOpR.slider;
    this.el.bgOpV = bgOpR.val;
    ...
    shapes.forEach((sh) => {
      ...
      b.addEventListener("click", () => {
        ...
        og.querySelectorAll(".p3d-obj-btn").forEach((x) => x.classList.remove("selected"));
    ...
    // Add to Scene button
    const addBtn = this._mkBtn(
      "+ Add to Scene",
      () => this._addObjectWithParams(),
      "p3d-btn p3d-btn-accent",
      "Add the selected shape with current parameters"
    );
    ...
    this._toolDefs.forEach((t) => {
      const b = document.createElement("div");
      b.className = "p3d-tool-btn" + (this.toolMode === t.id ? " active" : "");
      b.title = t.tip;
      b.innerHTML = `<span class="icon">${t.icon}</span>${t.l}`;
    ...
    cRow2.style.cssText = "display:flex;gap:5px;margin-top:5px;";
    // Perspective / Isometric switch
    const perspBtn = this._mkBtn(
      "Perspective",
      () => this._setPerspective(true),
      "p3d-btn active",
      "Standard perspective camera"
    );
    const isoBtn = this._mkBtn(
      "Isometric",
    ...
    const focusBtn = this._mkBtn(
      "\ud83d\udd0d Focus Selected (0)",
      () => this._camView("focus"),
      "p3d-btn",
      "Center camera on selected object (0 key)"
    );
    focusBtn.style.cssText = "width:100%;margin-top:5px;margin-bottom:8px;";
    cam.appendChild(focusBtn);
    // FOV
    ...
    this.el.fovVal = fovR.val;
    cam.appendChild(fovR.row);
    // Shadows toggle
    const shCheck = document.createElement("label");
    shCheck.className = "p3d-check";
    shCheck.title = "Toggle ground shadows";
    const shInp = document.createElement("input");
    shInp.type = "checkbox";
    shInp.checked = this._shadows;
    shInp.addEventListener("change", () => {
      this._shadows = shInp.checked;
      if (this._groundMesh) this._groundMesh.visible = shInp.checked;
    });
    ...
    // Grid toggle
    const gridCheck = document.createElement("label");
    gridCheck.className = "p3d-check";
    gridCheck.title = "Show/hide grid";
    const gridInp = document.createElement("input");
    gridInp.type = "checkbox";
    gridInp.checked = this._showGrid;
    gridInp.addEventListener("change", () => {
      this._showGrid = gridInp.checked;
      if (this.gridHelper) this.gridHelper.visible = gridInp.checked;
    });
    this.el.gridCheck = gridInp;
    gridCheck.append(gridInp, document.createTextNode("Show Grid"));
    gridCheck.style.marginTop = "6px";
    cam.appendChild(gridCheck);
    // Gizmo toggle
    const gizCheck = document.createElement("label");
    gizCheck.className = "p3d-check";
    gizCheck.title = "Show/hide transform gizmo";
    const gizInp = document.createElement("input");
    gizInp.type = "checkbox";
    gizInp.checked = this._showGizmo;
    gizInp.addEventListener("change", () => {
      this._showGizmo = gizInp.checked;
      if (this._gizmoHelper) this._gizmoHelper.visible = gizInp.checked;
    });
    this.el.gizmoCheck = gizInp;
    gizCheck.append(gizInp, document.createTextNode("Show Gizmo"));
    gizCheck.style.marginTop = "6px";
    cam.appendChild(gizCheck);
  ...
  _buildRightSidebar() {
    ...
    right.className = "p3d-right";

    // 1) Layers (first)
    const layers = this._section("Layers");
    const ll = document.createElement("div");
    ll.style.cssText = "max-height:200px;overflow-y:auto;";
    this.el.layerList = ll;
    layers.appendChild(ll);
    const lb = document.createElement("div");
    lb.style.cssText = "display:flex;gap:4px;margin-top:6px;";
    lb.append(
    ...
    colorIn.type = "color";
    colorIn.value = "#c4a882";
    colorIn.style.cssText =
      "width:100%;height:28px;cursor:pointer;border:1px solid #3a3d40;border-radius:4px;background:#111;";
    colorIn.addEventListener("input", () => this._setObjColor(colorIn.value));
    this.el.objColor = colorIn;
    colSec.appendChild(colorIn);
    // HSL sliders
    ...
    mats.appendChild(mg);
    const rR = this._sliderRow("Rough", 0, 100, 85, (v) => {
      for (const o of this.selectedObjs) o.material.roughness = v / 100;
      if (this.el.glossS) {
        const g = 100 - v;
        this.el.glossS.value = g;
        this.el.glossV.textContent = g;
      }
    });
    const gR = this._sliderRow("Gloss", 0, 100, 15, (v) => {
      for (const o of this.selectedObjs) o.material.roughness = 1 - v / 100;
      if (this.el.roughS) {
        const r = 100 - v;
        this.el.roughS.value = r;
        this.el.roughV.textContent = r;
      }
    });
    const oR = this._sliderRow("Opacity", 0, 100, 100, (v) => {
    ...
    this.el.roughV = rR.val;
    this.el.glossS = gR.slider;
    this.el.glossV = gR.val;
    this.el.opacS = oR.slider;
    this.el.opacV = oR.val;
    mats.append(rR.row, gR.row, oR.row);
    right.appendChild(mats);

    // 4) Lighting
    const lp = this._section("Lighting");
    const lcIn = document.createElement("input");
    lcIn.type = "color";
    lcIn.value = "#ffffff";
    lcIn.style.cssText =
      "width:50px;height:22px;cursor:pointer;border:1px solid #3a3d40;border-radius:4px;background:#111;";
    lcIn.addEventListener("input", () => {
      if (this.light) this.light.color.set(lcIn.value);
    });
    this.el.lightColor = lcIn;
    lp.appendChild(this._row("Color", lcIn));
    const iR = this._sliderRow("Intensity", 0, 200, 85, (v) => {
      if (this.light) this.light.intensity = (v / 100) * 2;
    });
    const sR = this._sliderRow("Ambient", 0, 100, 60, (v) => {
    ...
    lp.appendChild(dirLabel);
    const angR = this._sliderRow("Angle", 0, 360, 45, (v) => {
      this._lightDir.theta = (v * Math.PI) / 180;
      this._applyLightDir();
    });
    const hgtR = this._sliderRow("Height", 5, 90, 55, (v) => {
      this._lightDir.phi = ((90 - v) * Math.PI) / 180;
      this._applyLightDir();
    });
    this.el.lightAngle = angR.slider;
    this.el.lightAngleVal = angR.val;
    this.el.lightHeight = hgtR.slider;
    this.el.lightHeightVal = hgtR.val;
    lp.append(angR.row, hgtR.row);
    const resetLightBtn = this._mkBtn(
      "Reset Light",
      () => {
        this._lightDir = { theta: 0.8, phi: 1.0 };
        this._applyLightDir();
        if (this.el.lightAngle) {
          this.el.lightAngle.value = 45;
          this.el.lightAngleVal.textContent = 45;
        }
        if (this.el.lightHeight) {
          this.el.lightHeight.value = 55;
          this.el.lightHeightVal.textContent = 55;
        }
        if (this.light) {
    ...
    actSec.style.cssText =
      "padding:10px 12px;margin-top:auto;border-top:1px solid #2a2c2e;display:flex;flex-direction:column;gap:6px;flex-shrink:0;";
...
<b>Lock icon</b><span>Prevent transforms</span>
    ...
    cb.style.cssText = "position:absolute;top:8px;right:8px;";
    h.appendChild(cb);
    this.el.helpOverlay = h;
    return h;
  }
  ...
  _makeGeo(type, gp) {
    switch (type) {
      case "cube":
        return new THREE.BoxGeometry(gp.width || 1, gp.height || 1, gp.depth || 1);
      case "sphere":
        return new THREE.SphereGeometry(gp.radius || 0.6, gp.widthSegs || 16, gp.heightSegs || 16);
      case "cylinder":
      ...
      case "cone":
        return new THREE.ConeGeometry(gp.radius || 0.5, gp.height || 1.2, gp.sides || 16);
      case "torus":
        return new THREE.TorusGeometry(gp.radius || 0.5, gp.tube || 0.2, gp.radialSegs || 12, gp.tubeSegs || 32);
      case "plane":
  ...
  _handleKey(e) {
    ...
    const handled = ctrl
      ? ["z", "y", "d", "s"].includes(kl)
      : ["m", "r", "s", "f", "b", "t", "0", "1", "2", "3", "4", "delete", "backspace", "escape"].includes(kl);
    ...
    if (kl === "escape") {
      if (this.el.helpOverlay?.style.display === "block") this.el.helpOverlay.style.display = "none";
      return;
    }
  ...
  _selectAll() {
    ...
    this.objects.forEach((o) => {
      ...
      o.material.emissive?.setHex(0x3a1f00);
    });
  ...
  _updateLayers() {
    ...
    this.objects.forEach((obj, i) => {
      ...
      name.className = "p3d-layer-name";
      name.textContent = obj.userData.name || "Object";
      name.addEventListener("dblclick", (e) => {
        e.stopPropagation();
        const inp = document.createElement("input");
        inp.className = "p3d-name-input";
        inp.value = obj.userData.name;
        name.replaceWith(inp);
        inp.focus();
        inp.select();
        const fin = () => {
          obj.userData.name = inp.value.trim() || obj.userData.name;
      ...
      item.addEventListener("dragover", (e) => {
        ...
        list.querySelectorAll(".p3d-layer-item").forEach((it) => {
  ...
  _serializeScene() {
    return {
      ...
      camera: {
        ...
        target: { x: this.orbitCtrl.target.x, y: this.orbitCtrl.target.y, z: this.orbitCtrl.target.z },
      },
      light: {
        color: this.el.lightColor?.value || "#fff",
        intensity: this.light?.intensity || 1.7,
        ambient: this.ambientLight?.intensity || 0.6,
        dir: { ...this._lightDir },
      },
  ...
  _restoreScene(jsonStr) {
    ...
    try {
      ...
      if (d.doc_w) {
        this.docW = d.doc_w;
        if (this.el.wIn) this.el.wIn.value = d.doc_w;
      }
      if (d.doc_h) {
        this.docH = d.doc_h;
        if (this.el.hIn) this.el.hIn.value = d.doc_h;
      }
      ...
      if (this.el.ratioSel && this._ratios) {
        ...
        if (match) {
          this.el.ratioSel.value = match.v;
          this._activeRatio = match;
          this._ratioLocked = true;
        } else {
          this.el.ratioSel.value = "custom";
          this._activeRatio = null;
          this._ratioLocked = false;
        }
      }
      ...
      if (d.fov !== undefined) {
        ...
        if (this.el.fovVal) this.el.fovVal.textContent = d.fov;
  ...
  _syncBgSliders() {
    if (this.el.bgX) {
      ...
      this.el.bgXV.textContent = this._bgImg.x;
    }
    if (this.el.bgY) {
      this.el.bgY.value = this._bgImg.y;
      this.el.bgYV.textContent = this._bgImg.y;
    }
    if (this.el.bgSc) {
      this.el.bgSc.value = this._bgImg.scale;
      this.el.bgScV.textContent = this._bgImg.scale;
    }
    if (this.el.bgRot) {
      this.el.bgRot.value = this._bgImg.rotation;
      this.el.bgRotV.textContent = this._bgImg.rotation;
    }
    if (this.el.bgOp) {
      this.el.bgOp.value = this._bgImg.opacity;
      this.el.bgOpV.textContent = this._bgImg.opacity;
    }
  }
  ...
  _section(t) {
    ...
    s.className = "p3d-section";
    const h = document.createElement("div");
    h.className = "p3d-section-title";
    h.textContent = t;
    s.appendChild(h);
    return s;
  }
  ...
  _sliderRow(label, min, max, val, onChange) {
    ...
    return { row, slider, val: valEl };
  }
  _mkBtn(text, onClick, cls = "p3d-btn", tip = "") {
