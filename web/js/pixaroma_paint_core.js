...
export class PaintStudio extends PixaromaEditorBase {
  constructor() {
    super();
    this.docW = 1024;
    this.docH = 1024;
  ...
  _editorTitle() {
    return `Paint <span class="pxb-brand">Pixaroma</span>`;
  }
  ...
  _buildHelpStrip() {
    ...
    strip.className = "ppx-help-strip";
    strip.textContent =
      "B=Brush  P=Pencil  E=Eraser  G=Fill  I=Eyedrop  R=Smudge  V=Move  Space+Drag=Pan  Scroll=Zoom  Ctrl+Z=Undo  Ctrl+S=Save";
    this.el.helpStrip = strip;
    return strip;
  }
  ...
  _buildLeftSidebar() {
    ...
    toolLbl.className = "ppx-section-label";
    toolLbl.textContent = "Tools";
    ...
    colorLbl.className = "ppx-section-label";
    ...
    resetBtn.className = "ppx-swap-btn";
    resetBtn.title = "Reset to Black/White (D)";
    ...
    hslLbl.className = "ppx-section-label";
    ...
    swatchLbl.className = "ppx-section-label";
  ...
  _buildCanvasSection() {
    ...
    lbl.className = "ppx-section-label";
    lbl.textContent = "Canvas";
    sec.appendChild(lbl);

    // Aspect ratio dropdown (matching composer style)
    const ratioRow = document.createElement("div");
    ratioRow.style.cssText = "margin-bottom:4px;";
    const ratioSel = document.createElement("select");
    ratioSel.style.cssText =
      "width:100%;background:#111;color:#e0e0e0;border:1px solid #3a3d40;border-radius:3px;padding:2px 4px;font-size:10px;font-family:monospace;cursor:pointer;";
    ratioSel.innerHTML = `
    ...
    ratioSel.addEventListener("change", () => {
      ...
      if (dims) {
        this.el.docW.value = dims[0];
        this.el.docH.value = dims[1];
    ...
    bgRow.className = "ppx-bg-row";
    const bgLbl = document.createElement("label");
    bgLbl.textContent = "BG:";
    bgLbl.style.fontSize = "10px";
    const bgPrev = document.createElement("div");
    bgPrev.className = "ppx-bg-preview";
    bgPrev.style.background = this.bgColor;
...
Blend modes: Normal, Multiply, Screen, Overlay, Soft/Hard Light, Color Dodge/Burn, and more
  ...
  _buildRightSidebar() {
    ...
    const bmLbl = document.createElement("label");
    bmLbl.textContent = "Blend Mode";
    ...
    bmSel.addEventListener("change", () => {
      ...
      if (ly) {
        ly.blendMode = bmSel.value;
        this._renderDisplay();
      }
    });
    this.el.blendSel = bmSel;
    const opLbl = document.createElement("label");
    opLbl.textContent = "Layer Opacity";
    const opRow = document.createElement("div");
    opRow.style.cssText = "display:flex;align-items:center;gap:4px;";
    ...
    opNum.value = 100;
    opNum.style.cssText =
      "width:40px;background:#111;color:#e0e0e0;border:1px solid #3a3d40;border-radius:3px;padding:2px 3px;font-size:10px;font-family:monospace;";
    const opUpdate = (val) => {
      const ly = this.layers[this.activeIdx];
      if (!ly) return;
      ly.opacity = +val;
    ...
    actRow.style.cssText =
      "display:flex;gap:4px;padding:5px 7px;border-top:1px solid #2a2c2e;flex-shrink:0;flex-wrap:wrap;";
    const addBtn = this._mkBtn("+", () => this._addLayer(), "ppx-btn ppx-btn-sm");
    addBtn.title = "Add new layer";
    const dupBtn = this._mkBtn("\u2398", () => this._duplicateLayer(), "ppx-btn ppx-btn-sm");
    dupBtn.title = "Duplicate layer (Ctrl+D)";
    const delBtn = this._mkBtn("\ud83d\uddd1", () => this._deleteLayer(), "ppx-btn ppx-btn-sm ppx-btn-danger");
    delBtn.title = "Delete layer";
    const upBtn = this._mkBtn("\u25b2", () => this._moveLayer(-1), "ppx-btn ppx-btn-sm");
    upBtn.title = "Move layer up";
    const dnBtn = this._mkBtn("\u25bc", () => this._moveLayer(1), "ppx-btn ppx-btn-sm");
    ...
    tLbl.className = "ppx-section-label";
    tLbl.textContent = "Layer Transform";
    ...
    const mkSliderRow = (label, min, max, step, initVal) => {
      ...
      const lbl = document.createElement("div");
      lbl.style.cssText = "font-size:9px;color:#888;margin-bottom:1px;";
      lbl.textContent = label;
      const row = document.createElement("div");
      ...
      num.value = initVal;
      num.style.cssText =
        "width:48px;background:#111;color:#e0e0e0;border:1px solid #3a3d40;border-radius:3px;padding:2px 3px;font-size:10px;font-family:monospace;";
    ...
    const rotUpdate = (val) => {
      ...
      this.el.txRot.slider.value = val;
      this.el.txRot.num.value = val;
      this._renderDisplay();
    };
    ...
    const scaleUpdate = (val) => {
      ...
      this.el.txScale.slider.value = val;
      this.el.txScale.num.value = val;
      this.el.txStretchH.slider.value = val;
      this.el.txStretchH.num.value = val;
      this.el.txStretchV.slider.value = val;
      this.el.txStretchV.num.value = val;
      this._renderDisplay();
    };
    ...
    fitRow.style.cssText = "display:flex;gap:4px;margin-bottom:3px;";
    const fitW = this._mkBtn(
      "↔ Fit Width",
      () => {
        const ly = this.layers[this.activeIdx];
        if (!ly) return;
    ...
    flipRow.style.cssText = "display:flex;gap:4px;margin-bottom:3px;";
    const flipH = this._mkBtn(
      "◧ Flip H",
      () => {
        const ly = this.layers[this.activeIdx];
        if (ly) {
    ...
    rotBtnRow.style.cssText = "display:flex;gap:4px;margin-bottom:3px;";
    const rotL = this._mkBtn(
      "↺ -90°",
      () => {
        const ly = this.layers[this.activeIdx];
        if (!ly) return;
    ...
    resetT.style.cssText = "width:100%;margin-top:3px;";
    ...
    const clearLyrBtn = this._mkBtn(
      "\ud83d\uddd1 Clear Layer",
      () => this._clearLayer(),
      "ppx-btn ppx-btn-danger",
      "Erase all pixels on the active layer"
    );
    const flattenBtn = this._mkBtn(
      "\u2b07 Flatten All",
      () => this._flattenAll(),
      "ppx-btn",
    ...
    helpB.style.cssText =
      "width:100%;padding:7px 0;font-size:13px;border-radius:5px;background:#353535;border:1px solid #3a3d40;color:#ccc;";
    ...
    closeB.style.cssText =
      "flex:1;padding:7px 0;font-size:13px;border-radius:5px;background:#353535;border:1px solid #3a3d40;color:#ccc;";
  ...
  _deleteLayer() {
    ...
    if (toDelete.length >= this.layers.length) {
      this._setStatus("Cannot delete all layers");
      return;
    }
  ...
  async _loadLayers(layersData) {
    ...
    for (const ld of layersData) {
      ...
      ly.transform = ld.transform
        ...
        : { x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0, flipX: false, flipY: false, pivotOffX: 0, pivotOffY: 0 };
      if (ld.src) {
          ...
          if (d[3] > 8) {
            if (e.ctrlKey || e.metaKey) {
              ...
              if (this.selectedIndices.has(i)) {
                ...
                if (this.activeIdx === i)
                  this.activeIdx = this.selectedIndices.size > 0 ? [...this.selectedIndices][0] : i;
              } else {
  ...
  _handleKeyDown(e) {
    ...
    const handled = e.ctrlKey
      ...
      : ["b", "p", "e", "g", "i", "r", "v", "t", "u", "x", "d", "[", "]", "delete", "enter", "escape", "?"].includes(
          key
        );
  ...
  _showBgColorPicker(e, anchor) {
    ...
    inp.style.cssText = "width:100%;height:28px;cursor:pointer;border:none;background:none;";
  ...
  _updateToolOptions() {
    ...
    if (this.tool === "brush" || this.tool === "pencil" || this.tool === "eraser" || this.tool === "smudge") {
      const resetBtn = document.createElement("button");
      resetBtn.className = "ppx-btn ppx-btn-sm";
      ...
      if (this.tool === "brush" || this.tool === "pencil") {
        ...
        angSlide.style.cssText = "width:60px;accent-color:#f66744;cursor:pointer;flex-shrink:0;";
        ...
        SHAPES.forEach((sh) => {
          ...
          btn.addEventListener("click", () => {
            ...
            this.engine._stampKey = "";
            bar.querySelectorAll(".ppx-shape-btn").forEach((b) => b.classList.remove("active"));
    ...
    if (this.tool === "pick") {
      ...
      lbl.textContent = "Click or drag to sample color  ·  Alt+click picks to background color";
      bar.appendChild(lbl);
    }
    ...
    if (this.tool === "shape") {
      ...
      const updateShapeOptions = () => {
        ...
        if (this._shapeTool === "poly") {
          ...
          sidesSlide.style.cssText = "width:60px;accent-color:#f66744;";
        ...
        if (!this.shapeFill || this._shapeTool === "line") {
          ...
          swSlide.style.cssText = "width:60px;accent-color:#f66744;";
  ...
  _updateLayersPanel() {
    ...
    this.layers.forEach((ly, i) => {
      ...
      item.className =
        "ppx-layer-item" +
        (i === this.activeIdx ? " active" : "") +
        (this.selectedIndices.has(i) && i !== this.activeIdx ? " multi-selected" : "");
      item.title = ly.name;
      item.dataset.idx = i;

      const vis = document.createElement("div");
      vis.className = "ppx-layer-vis" + (ly.visible ? " on" : "");
      vis.textContent = ly.visible ? "\ud83d\udc41" : "\u25cc";
      vis.title = "Toggle visibility";
      vis.addEventListener("click", (e) => {
        e.stopPropagation();
        ly.visible = !ly.visible;
        vis.textContent = ly.visible ? "\ud83d\udc41" : "\u25cc";
        vis.classList.toggle("on", ly.visible);
        this._renderDisplay();
      });

      const thumb = document.createElement("div");
      thumb.className = "ppx-layer-thumb";
      ...
      name.className = "ppx-layer-name";
      name.textContent = ly.name;
      name.addEventListener("dblclick", (e) => {
        e.stopPropagation();
        const inp = document.createElement("input");
        inp.className = "ppx-layer-name-input";
        inp.value = ly.name;
        name.replaceWith(inp);
        inp.focus();
        inp.select();
        const finish = () => {
          ly.name = inp.value.trim() || ly.name;
      ...
      lock.title = ly.locked ? "Unlock" : "Lock layer";
      lock.addEventListener("click", (e) => {
        e.stopPropagation();
        ly.locked = !ly.locked;
        lock.textContent = ly.locked ? "\ud83d\udd12" : "\ud83d\udd13";
        lock.classList.toggle("locked", ly.locked);
      });
      ...
      item.addEventListener("dragend", () => {
        item.classList.remove("ppx-dragging");
      });
      item.addEventListener("dragover", (e) => {
        e.preventDefault();
        e.dataTransfer.dropEffect = "move";
        const mid = item.getBoundingClientRect().top + item.getBoundingClientRect().height / 2;
        list.querySelectorAll(".ppx-layer-item").forEach((it) => {
          it.classList.remove("ppx-drag-over-top", "ppx-drag-over-bottom");
        });
  ...
  _syncTransformPanel() {
    ...
    if (this.el.txRot) {
      ...
      this.el.txRot.slider.value = deg;
      this.el.txRot.num.value = deg;
    }
    if (this.el.txScale) {
      const pct = Math.round(t.scaleX * 100);
      this.el.txScale.slider.value = pct;
      this.el.txScale.num.value = pct;
    }
    if (this.el.txStretchH) {
      const pct = Math.round(t.scaleX * 100);
      this.el.txStretchH.slider.value = pct;
      this.el.txStretchH.num.value = pct;
    }
    if (this.el.txStretchV) {
      const pct = Math.round(t.scaleY * 100);
      this.el.txStretchV.slider.value = pct;
      this.el.txStretchV.num.value = pct;
    }
  }
