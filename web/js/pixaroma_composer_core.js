...
export class PixaromaEditor {
    ...
    attachEvents() {
        ...
        const syncDocResize = () => {
            ...
            this.fitViewToWorkspace(); this.draw(); this.pushHistory();
        };

        this.ratioSelect.addEventListener("change", (e) => {
            const val = e.target.value; 
            if (val === "custom") { this.ratioLock = false; } else {
                this.ratioLock = true; const [rw, rh] = val.split(":").map(Number); this.ratioValue = rw / rh;
                this.docHeight = Math.round(this.docWidth / this.ratioValue); this.docHInput.value = this.docHeight; 
                syncDocResize();
            }
        });
        ...
        this.docWInput.addEventListener("change", () => { 
            ...
            if(this.ratioLock) { this.docHeight = Math.round(this.docWidth / this.ratioValue); this.docHInput.value = this.docHeight; }
            syncDocResize();
        });

        this.docHInput.addEventListener("change", () => { 
            this.docHeight = parseInt(this.docHInput.value) || 1024; 
            if(this.ratioLock) { this.docWidth = Math.round(this.docHeight * this.ratioValue); this.docWInput.value = this.docWidth; }
            syncDocResize();
        });
        ...
        this.removeBgBtn.addEventListener("click", async () => {
            if (this.selectedLayerIds.size === 0) return;
            const originalText = this.removeBgBtn.innerText; this.removeBgBtn.innerText = "⏳ ML Processing..."; this.removeBgBtn.disabled = true;
            const targetId = Array.from(this.selectedLayerIds)[0];
            const layer = this.layers.find(l => l.id === targetId);
            ...
            try {
                ...
                } else if (data.error) { alert("ML Error: " + data.error); }
                else { const newImg = new Image(); newImg.crossOrigin = "Anonymous"; newImg.onload = () => { layer.img = newImg; layer.rawB64_internal = data.image; layer.savedOnServer = false; this.draw(); this.pushHistory(); }; newImg.src = data.image; }
            } catch (err) { alert("Failed ML backend."); } finally { this.removeBgBtn.innerText = originalText; this.removeBgBtn.disabled = false; }
        });
    ...
    async attemptRestore() {
        ...
        try {
            ...
            this.docWInput.value = meta.doc_w; this.docHInput.value = meta.doc_h;
