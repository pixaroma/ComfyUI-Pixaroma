...
.pcrop-overlay {
    ...
    background:#171718; font-family:'Segoe UI',system-ui,sans-serif;
...
.pcrop-title { color:#fff; font-size:13px; font-weight:bold; flex:1; display:flex; align-items:center; gap:6px; }
.pcrop-body { display:flex; flex:1; overflow:hidden; min-height:0; }
.pcrop-sidebar {
...
export class CropEditor {
    ...
    open(jsonStr) {
        ...
        if (this._srcPath) {
            ...
            this._loadImageFromURL(url, () => {
                ...
                if (data.ratio_idx != null) { this.ratioIdx = data.ratio_idx;  this._updateRatioPills(); }
                if (data.snap_idx != null) { this.snapIdx = data.snap_idx; this._updateSnapPills(); }
    ...
    // ─── Build UI ────────────────────────────────────────────
    _buildUI() {
        const ov = document.createElement("div"); ov.className = "pcrop-overlay"; this.el.overlay = ov;

        // Titlebar (just logo + title, no buttons)
        const tb = document.createElement("div"); tb.className = "pcrop-titlebar";
        const title = document.createElement("span"); title.className = "pcrop-title";
        title.innerHTML = `<img src="/pixaroma/assets/pixaroma_logo.svg" style="width:20px;height:20px;">Image Crop <span style="color:${BRAND}">Pixaroma</span>`;
        tb.appendChild(title);
        ov.appendChild(tb);

        // Body
        const body = document.createElement("div"); body.className = "pcrop-body";
        body.appendChild(this._buildSidebar());

        // Workspace
        const ws = document.createElement("div"); ws.className = "pcrop-workspace"; this.el.workspace = ws;
        const wrap = document.createElement("div"); wrap.className = "pcrop-canvas-wrap"; this.el.canvasWrap = wrap;
        const cvs = document.createElement("canvas"); cvs.width = 100; cvs.height = 100;
        this.el.canvas = cvs; this.el.ctx = cvs.getContext("2d");
        wrap.appendChild(cvs);

        // Help overlay
        const help = document.createElement("div"); help.className = "pcrop-help-overlay";
        help.innerHTML = `
...
`;
        this.el.help = help; ws.appendChild(help);
        ws.appendChild(wrap);
        body.appendChild(ws);
        ov.appendChild(body);

        // Bottom bar
        const bb = document.createElement("div"); bb.className = "pcrop-bottombar";
        const status = document.createElement("span"); status.className = "pcrop-status"; status.textContent = "Load an image to begin";
        this.el.status = status; bb.appendChild(status);
        ov.appendChild(bb);
    ...
    // ─── Sidebar ─────────────────────────────────────────────
    _buildSidebar() {
        const sb = document.createElement("div"); sb.className = "pcrop-sidebar";
        ...
        const ratioGrid = document.createElement("div"); ratioGrid.className = "pcrop-pill-grid";
        this.el.ratioPills = [];
        RATIOS.forEach((r, i) => {
            const pill = document.createElement("button");
            pill.className = "pcrop-pill" + (i === 0 ? " active" : "");
            pill.textContent = r.label;
            pill.onclick = () => { this.ratioIdx = i; this.ratioSwapped = false; this._applyRatio(); this._updateRatioPills(); };
            ratioGrid.appendChild(pill); this.el.ratioPills.push(pill);
        });
        secRatio.appendChild(ratioGrid);
        const swapBtn = document.createElement("button");
        swapBtn.className = "pcrop-pill"; swapBtn.textContent = "⇄ Swap W↔H";
        ...
        const snapGrid = document.createElement("div"); snapGrid.className = "pcrop-pill-grid";
        this.el.snapPills = [];
        SNAPS.forEach((s, i) => {
            const pill = document.createElement("button");
            pill.className = "pcrop-pill" + (i === 0 ? " active" : "");
            pill.textContent = s.label;
            pill.onclick = () => { this.snapIdx = i; this._applySnap(); this._updateSnapPills(); };
            snapGrid.appendChild(pill); this.el.snapPills.push(pill);
        });
        ...
        const mkSlider = (label, key) => {
            const row = document.createElement("div"); row.className = "pcrop-slider-row";
            const lbl = document.createElement("label"); lbl.textContent = label; row.appendChild(lbl);
            const slider = document.createElement("input"); slider.type = "range"; slider.min = "1"; slider.max = "4096"; slider.value = "1024";
            const num = document.createElement("input"); num.type = "number"; num.min = "1"; num.max = "4096";
            slider.addEventListener("input", () => { num.value = slider.value; this._onSliderChange(key); });
            num.addEventListener("input", () => { slider.value = num.value; this._onSliderChange(key); });
            row.appendChild(slider); row.appendChild(num);
            return { row, slider, num };
        };
        const sW = mkSlider("W", "w"); const sH = mkSlider("H", "h");
        const sX = mkSlider("X", "x"); const sY = mkSlider("Y", "y");
        this.el.sliderW = sW; this.el.sliderH = sH; this.el.sliderX = sX; this.el.sliderY = sY;
        secSize.appendChild(sW.row); secSize.appendChild(sH.row);
        secSize.appendChild(sX.row); secSize.appendChild(sY.row);
        sb.appendChild(secSize);

        // Info
        const secInfo = this._mkSection("Info");
        const infoDiv = document.createElement("div"); infoDiv.className = "pcrop-info"; infoDiv.innerHTML = "No image loaded";
        this.el.info = infoDiv; secInfo.appendChild(infoDiv);
        ...
        helpB.style.cssText = "width:100%;padding:7px 0;font-size:13px;border-radius:5px;background:#353535;border:1px solid #3a3d40;color:#ccc;text-align:center;";
        ...
        closeB.style.cssText = "flex:1;padding:7px 0;font-size:13px;border-radius:5px;background:#353535;border:1px solid #3a3d40;color:#ccc;text-align:center;";
    ...
    _mkSection(title) {
        const sec = document.createElement("div"); sec.className = "pcrop-section";
        const t = document.createElement("div"); t.className = "pcrop-section-title"; t.textContent = title;
        sec.appendChild(t); return sec;
    }

    _mkBtn(text, onClick, cls = "pcrop-btn") {
        const btn = document.createElement("button"); btn.className = cls; btn.textContent = text;
        btn.addEventListener("click", onClick); return btn;
    }

    _updateRatioPills() { this.el.ratioPills.forEach((p, i) => p.classList.toggle("active", i === this.ratioIdx)); }
    _updateSnapPills() { this.el.snapPills.forEach((p, i) => p.classList.toggle("active", i === this.snapIdx)); }
    ...
    // ─── Info & Sliders ──────────────────────────────────────
    _updateInfo() {
        if (!this.img) { this.el.info.innerHTML = "No image loaded"; return; }
    ...
    _setSlider(s, val, min, max) {
        s.slider.min = min; s.slider.max = max; s.slider.value = val;
        s.num.min = min; s.num.max = max; s.num.value = val;
    }
    ...
    _bindKeys() {
        this._keyHandler = (e) => {
            ...
            if (key === "f" && !ctrl) { e.preventDefault(); this.ratioIdx = 0; this._updateRatioPills(); this._draw(); this._updateInfo(); return; }
    ...
    _toggleHelp() { const h = this.el.help; h.style.display = h.style.display === "block" ? "none" : "block"; }
    _setStatus(msg) { if (this.el.status) this.el.status.textContent = msg; }

    // ─── Save ────────────────────────────────────────────────
    async _save() {
        if (!this.img) { this._setStatus("No image to save"); return; }
        this.el.saveBtn.disabled = true; this.el.saveBtn.textContent = "⏳ Saving...";
        ...
        try {
            ...
            this.el.saveBtn.disabled = false; this.el.saveBtn.textContent = "Save";
        }
    }
}
