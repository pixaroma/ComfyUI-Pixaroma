...
export class BrushEngine {
    ...
    _buildStamp(size, hardness, shape, angle) {
        ...
        switch (shape) {
            case "round":    this._stampRound(ctx, r, h01);    break;
            case "square":   this._stampSquare(ctx, r, h01);   break;
            case "triangle": this._stampTri(ctx, r, h01);      break;
            case "diamond":  this._stampDiamond(ctx, r, h01);  break;
            case "star":     this._stampStar(ctx, r, h01);     break;
            case "flat":     this._stampFlat(ctx, r, h01);     break;
            case "leaf":     this._stampLeaf(ctx, r, h01);     break;
            default:         this._stampRound(ctx, r, h01);
        }
        ctx.restore();
    ...
    applyStampToCtx(ctx, stamp, x, y, size, color, flowAlpha, isEraser, scatter, scatterAmt) {
        const half = stamp.width / 2;
        const sx   = scatterAmt > 0 ? x + (Math.random() - 0.5) * scatterAmt * size : x;
        const sy   = scatterAmt > 0 ? y + (Math.random() - 0.5) * scatterAmt * size : y;
    ...
    floodFill(canvas, startX, startY, fillColor, tolerance) {
        ...
        while (stack.length > 0) {
            ...
            while (rx < w) {
                const i = (cy * w + rx) * 4;
                if (!match(i)) break;
                if (visited[cy * w + rx]) { rx++; continue; }
                set(i);
                visited[cy * w + rx] = 1;
                ...
                if (cy > 0) {
                    ...
                    if (!spanUp && match(ni) && !visited[(cy-1)*w+rx]) {
                        stack.push([rx, cy - 1]); spanUp = true;
                ...
                if (cy < h - 1) {
                    ...
                    if (!spanDn && match(ni) && !visited[(cy+1)*w+rx]) {
                        stack.push([rx, cy + 1]); spanDn = true;
    ...
    smudgeBegin(ctx, x, y, size) {
        ...
        if (sw <= 0 || sh <= 0) { this._smudgePatch = null; return; }
