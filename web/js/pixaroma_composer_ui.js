...
export class PixaromaUI {
    ...
    refreshLayersPanel() {
        ...
        [...core.layers].reverse().forEach((layer, visualIndex) => {
            ...
            item.className = `pix-layer-item${isFirst ? ' active' : ''}${isSelected && !isFirst ? ' multi-selected' : ''}${layer.locked ? ' locked' : ''}`;
            ...
            // --- Thumbnail ---
            const thumb = document.createElement("div"); thumb.className = "pix-layer-thumb";
            ...
            const name = document.createElement("div"); name.className = "pix-layer-name"; name.textContent = layer.name;
            name.addEventListener("dblclick", (e) => {
                e.stopPropagation();
                const inp = document.createElement("input");
                inp.className = "pix-layer-rename-input"; inp.value = layer.name;
                name.replaceWith(inp); inp.focus(); inp.select();
                const finish = () => { layer.name = inp.value.trim() || layer.name; inp.replaceWith(name); name.textContent = layer.name; core.pushHistory(); };
            ...
            item.addEventListener("click", (e) => {
                ...
                if (e.target.classList.contains('pix-layer-vis') || e.target.classList.contains('pix-layer-lock') || e.target.tagName === 'INPUT') return;
            ...
            item.addEventListener("dragend", () => { item.classList.remove("pix-dragging"); });
            item.addEventListener("dragover", (e) => {
                e.preventDefault(); e.dataTransfer.dropEffect = "move";
                const mid = item.getBoundingClientRect().top + item.getBoundingClientRect().height / 2;
                core.layersList.querySelectorAll(".pix-layer-item").forEach(it => { it.classList.remove("pix-drag-over-top", "pix-drag-over-bottom"); });
                item.classList.add(e.clientY < mid ? "pix-drag-over-top" : "pix-drag-over-bottom");
            });
            item.addEventListener("dragleave", () => { item.classList.remove("pix-drag-over-top", "pix-drag-over-bottom"); });
    ...
    build() {
        ...
        if (!document.getElementById("pixaroma-styles-v14")) {
            const style = document.createElement("style"); style.id = "pixaroma-styles-v14";
            style.innerHTML = `
        ...
        // LEFT SIDEBAR
        // =====================================================================
        const sidebarLeft = document.createElement("div"); sidebarLeft.className = "pix-sidebar-left";
        ...
        const docPanel = document.createElement("div"); docPanel.className = "pix-panel";
        docPanel.innerHTML = `<div class="pix-panel-title">Document Setup</div>`;

        const ratioRow = document.createElement("div"); ratioRow.className = "pix-row";
        core.ratioSelect = document.createElement("select"); core.ratioSelect.className = "pix-input";
        ...
        const docRow = document.createElement("div"); docRow.className = "pix-row";
        const colW = document.createElement("div"); colW.className = "pix-col"; colW.innerHTML = `<div class="pix-label">Width (px)</div>`;
        core.docWInput = document.createElement("input"); core.docWInput.type = "number"; core.docWInput.className = "pix-input"; core.docWInput.value = core.docWidth; colW.appendChild(core.docWInput);
        const colH = document.createElement("div"); colH.className = "pix-col"; colH.innerHTML = `<div class="pix-label">Height (px)</div>`;
        core.docHInput = document.createElement("input"); core.docHInput.type = "number"; core.docHInput.className = "pix-input"; core.docHInput.value = core.docHeight; colH.appendChild(core.docHInput);
        docRow.append(colW, colH);
        docPanel.append(ratioRow, docRow);
        sidebarLeft.appendChild(docPanel);

        // --- 2. Workspace Content (add images) ---
        const addPanel = document.createElement("div"); addPanel.className = "pix-panel";
        ...
        const uploadTrigger = document.createElement("button"); uploadTrigger.className = "pix-btn"; uploadTrigger.style.width = "100%"; uploadTrigger.innerHTML = `🖼️ Add Image Layer`;
        uploadTrigger.onclick = () => core.uploadBtn.click();
        const clearBtn = document.createElement("button"); clearBtn.className = "pix-btn"; clearBtn.style.width = "100%"; clearBtn.style.marginTop = "8px"; clearBtn.innerHTML = `🗑️ Clear Canvas`;
        clearBtn.onclick = () => { core.layers = []; core.selectedLayerIds.clear(); core.syncActiveLayerIndex(); this.updateActiveLayerUI(); core.draw(); core.pushHistory(); };
        ...
        core.toolsPanel = document.createElement("div"); core.toolsPanel.className = "pix-panel";
        ...
        const fitRow = document.createElement("div"); fitRow.className = "pix-row";
        core.btnFitW = document.createElement("button"); core.btnFitW.className = "pix-btn"; core.btnFitW.innerText = "↔ Fit Width";
        core.btnFitH = document.createElement("button"); core.btnFitH.className = "pix-btn"; core.btnFitH.innerText = "↕ Fit Height";
        fitRow.append(core.btnFitW, core.btnFitH);

        const flipRow = document.createElement("div"); flipRow.className = "pix-row";
        core.btnFlipH = document.createElement("button"); core.btnFlipH.className = "pix-btn"; core.btnFlipH.innerText = "◧ Flip H";
        core.btnFlipV = document.createElement("button"); core.btnFlipV.className = "pix-btn"; core.btnFlipV.innerText = "⬒ Flip V";
        flipRow.append(core.btnFlipH, core.btnFlipV);

        const rotRow = document.createElement("div"); rotRow.className = "pix-row";
        core.btnRotLeft = document.createElement("button"); core.btnRotLeft.className = "pix-btn"; core.btnRotLeft.innerText = "↺ -90°";
        core.btnRotRight = document.createElement("button"); core.btnRotRight.className = "pix-btn"; core.btnRotRight.innerText = "↻ +90°";
        rotRow.append(core.btnRotLeft, core.btnRotRight);

        core.btnReset = document.createElement("button"); core.btnReset.className = "pix-btn"; core.btnReset.innerText = "🔄 Reset Transform";
        core.btnReset.style.width = "100%"; core.btnReset.style.marginBottom = "15px";

        const rangeRow = document.createElement("div"); rangeRow.className = "pix-panel"; rangeRow.style.padding = "10px"; rangeRow.style.background = "#131415";

        const rotateLabel = document.createElement("div"); rotateLabel.className = "pix-label"; rotateLabel.innerText = "Rotate (degrees)";
        const rotWrap = document.createElement("div"); rotWrap.className = "pix-row";
        core.rotateSlider = document.createElement("input"); core.rotateSlider.type = "range"; core.rotateSlider.min = "0"; core.rotateSlider.max = "360"; core.rotateSlider.step = "1"; core.rotateSlider.value = "0";
        core.rotateNum = document.createElement("input"); core.rotateNum.type = "number"; core.rotateNum.className = "pix-input pix-input-num"; core.rotateNum.min = "0"; core.rotateNum.max = "360";
        rotWrap.append(core.rotateSlider, core.rotateNum);

        const scaleLabel = document.createElement("div"); scaleLabel.className = "pix-label"; scaleLabel.innerText = "Uniform Scale (%)"; scaleLabel.style.marginTop = "10px";
        const scaleWrap = document.createElement("div"); scaleWrap.className = "pix-row";
        core.scaleSlider = document.createElement("input"); core.scaleSlider.type = "range"; core.scaleSlider.min = "5"; core.scaleSlider.max = "300"; core.scaleSlider.step = "1"; core.scaleSlider.value = "100";
        core.scaleNum = document.createElement("input"); core.scaleNum.type = "number"; core.scaleNum.className = "pix-input pix-input-num"; core.scaleNum.min = "5"; core.scaleNum.max = "300";
        scaleWrap.append(core.scaleSlider, core.scaleNum);

        const stretchRow = document.createElement("div"); stretchRow.className = "pix-row"; stretchRow.style.marginTop = "10px"; stretchRow.style.borderTop = "1px solid #3a3d40"; stretchRow.style.paddingTop = "10px";
        const colSH = document.createElement("div"); colSH.className = "pix-col"; colSH.innerHTML = `<div class="pix-label">Stretch Horiz (%)</div>`;
        const wrapSH = document.createElement("div"); wrapSH.className = "pix-row";
        core.stretchHSlider = document.createElement("input"); core.stretchHSlider.type = "range"; core.stretchHSlider.min = "5"; core.stretchHSlider.max = "300"; core.stretchHSlider.step = "1"; core.stretchHSlider.value = "100";
        core.stretchHNum = document.createElement("input"); core.stretchHNum.type = "number"; core.stretchHNum.className = "pix-input pix-input-num"; core.stretchHNum.value = "100";
        wrapSH.append(core.stretchHSlider, core.stretchHNum); colSH.appendChild(wrapSH);
        stretchRow.append(colSH);

        const stretchRowV = document.createElement("div"); stretchRowV.className = "pix-row";
        const colSV = document.createElement("div"); colSV.className = "pix-col"; colSV.innerHTML = `<div class="pix-label">Stretch Vert (%)</div>`;
        const wrapSV = document.createElement("div"); wrapSV.className = "pix-row";
        core.stretchVSlider = document.createElement("input"); core.stretchVSlider.type = "range"; core.stretchVSlider.min = "5"; core.stretchVSlider.max = "300"; core.stretchVSlider.step = "1"; core.stretchVSlider.value = "100";
        core.stretchVNum = document.createElement("input"); core.stretchVNum.type = "number"; core.stretchVNum.className = "pix-input pix-input-num"; core.stretchVNum.value = "100";
        wrapSV.append(core.stretchVSlider, core.stretchVNum); colSV.appendChild(wrapSV);
        stretchRowV.append(colSV);

        const opacityLabel = document.createElement("div"); opacityLabel.className = "pix-label"; opacityLabel.innerText = "Opacity (%)"; opacityLabel.style.marginTop = "10px";
        const opWrap = document.createElement("div"); opWrap.className = "pix-row";
        core.opacitySlider = document.createElement("input"); core.opacitySlider.type = "range"; core.opacitySlider.min = "0"; core.opacitySlider.max = "100"; core.opacitySlider.step = "1"; core.opacitySlider.value = "100";
        core.opacityNum = document.createElement("input"); core.opacityNum.type = "number"; core.opacityNum.className = "pix-input pix-input-num"; core.opacityNum.min = "0"; core.opacityNum.max = "100";
        ...
        core.statusText.style.cssText = "font-size:11px; color:#888; padding:10px 4px 4px 4px; text-align:center; margin-top:auto; border-top:1px solid #3a3d40;";
        ...
        // CANVAS / WORKSPACE
        // =====================================================================
        core.workspace = document.createElement("div"); core.workspace.className = "pix-workspace";
        ...
        const alignBar = document.createElement("div"); alignBar.className = "pix-align-bar";
        alignBar.innerHTML = `
        ...
        const topBar = document.createElement("div"); topBar.className = "pix-top-bar";
        core.btnUndo = document.createElement("button"); core.btnUndo.className = "pix-view-btn"; core.btnUndo.innerHTML = "↩ Undo"; core.btnUndo.style.background = "rgba(36,38,40,0.9)";
        core.btnRedo = document.createElement("button"); core.btnRedo.className = "pix-view-btn"; core.btnRedo.innerHTML = "↪ Redo"; core.btnRedo.style.background = "rgba(36,38,40,0.9)";
        core.btnHelp = document.createElement("button"); core.btnHelp.className = "pix-view-btn"; core.btnHelp.innerHTML = "❓ Help"; core.btnHelp.style.background = "rgba(36,38,40,0.9)";
        ...
        core.helpPanel.style.cssText = "display: none; position: absolute; top: 65px; right: 20px; width: 300px; background: rgba(36,38,40,0.95); border: 1px solid #3a3d40; border-radius: 8px; padding: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.8); z-index: 100; backdrop-filter: blur(5px);";
        ...
        core.btnHelp.onclick = () => { core.helpPanel.style.display = core.helpPanel.style.display === 'none' ? 'block' : 'none'; };
        core.helpPanel.querySelector('#pix-close-help').onclick = () => { core.helpPanel.style.display = 'none'; };

        // Zoom controls (bottom-center of workspace)
        const viewControls = document.createElement("div"); viewControls.className = "pix-view-controls";
        const btnZoomOut = document.createElement("button"); btnZoomOut.className = "pix-view-btn"; btnZoomOut.innerHTML = "➖"; btnZoomOut.title = "Zoom Out";
        const btnZoomFit = document.createElement("button"); btnZoomFit.className = "pix-view-btn"; btnZoomFit.innerHTML = "🔲 Fit View"; btnZoomFit.title = "Fit to Screen";
        const btnZoomIn = document.createElement("button"); btnZoomIn.className = "pix-view-btn"; btnZoomIn.innerHTML = "➕"; btnZoomIn.title = "Zoom In";
        btnZoomOut.onclick = () => { core.viewZoom *= 0.8; core.updateViewTransform(); };
        btnZoomIn.onclick = () => { core.viewZoom *= 1.2; core.updateViewTransform(); };
        ...
        // RIGHT SIDEBAR
        // =====================================================================
        const sidebarRight = document.createElement("div"); sidebarRight.className = "pix-sidebar-right";

        // --- 1. Layers Stack ---
        core.layersListPanel = document.createElement("div"); core.layersListPanel.className = "pix-panel";
        core.layersListPanel.style.flex = "1"; core.layersListPanel.style.display = "flex"; core.layersListPanel.style.flexDirection = "column";
        core.layersListPanel.innerHTML = `<div class="pix-panel-title">Layers</div>`;
        core.layersList = document.createElement("div"); core.layersList.className = "pix-layers-list";
        core.layersListPanel.appendChild(core.layersList);

        // Layer action buttons row (like Paint node)
        const actRow = document.createElement("div"); actRow.className = "pix-layer-actions";
        const mkBtn = (label, fn, cls) => { const b = document.createElement("button"); b.className = cls || "pix-btn"; b.innerHTML = label; b.addEventListener("click", fn); return b; };
        const addBtn = mkBtn("+", () => core.uploadBtn.click()); addBtn.title = "Add image layer";
        core.btnDupLayer = mkBtn("\u2398", () => {}); core.btnDupLayer.title = "Duplicate layer"; core.btnDupLayer.style.opacity = "0.3";
        core.btnDelLayer = mkBtn("\ud83d\uddd1", () => {}, "pix-btn pix-btn-danger"); core.btnDelLayer.title = "Delete layer"; core.btnDelLayer.style.opacity = "0.3";
        const upBtn = mkBtn("\u25b2", () => this.moveLayer(1)); upBtn.title = "Move layer up";
        const dnBtn = mkBtn("\u25bc", () => this.moveLayer(-1)); dnBtn.title = "Move layer down";
        ...
        core.eraserPanel = document.createElement("div"); core.eraserPanel.className = "pix-panel";
        core.eraserPanel.style.opacity = "0.3"; core.eraserPanel.style.pointerEvents = "none";
        core.eraserPanel.style.marginBottom = "0";

        const eraserTitleRow = document.createElement("div"); eraserTitleRow.style.display = "flex"; eraserTitleRow.style.justifyContent = "space-between"; eraserTitleRow.style.alignItems = "center"; eraserTitleRow.style.marginBottom = "12px";
        const eraserTitle = document.createElement("div"); eraserTitle.className = "pix-panel-title"; eraserTitle.style.margin = "0"; eraserTitle.innerText = "ERASER";

        // Toggle eraser on/off — replaces the old tool tab
        core.btnEraserToggle = document.createElement("button"); core.btnEraserToggle.className = "pix-btn"; core.btnEraserToggle.style.flex = "0 0 auto"; core.btnEraserToggle.style.fontSize = "11px"; core.btnEraserToggle.style.padding = "4px 10px";
        ...
        // Brush Size
        const sizeLabel = document.createElement("div"); sizeLabel.className = "pix-label"; sizeLabel.innerText = "Brush Size";
        const sizeWrap = document.createElement("div"); sizeWrap.className = "pix-row"; sizeWrap.style.marginBottom = "10px";
        core.brushSizeSlider = document.createElement("input"); core.brushSizeSlider.type = "range"; core.brushSizeSlider.min = "1"; core.brushSizeSlider.max = "200"; core.brushSizeSlider.value = core.brushSize;
        core.brushSizeNum = document.createElement("input"); core.brushSizeNum.type = "number"; core.brushSizeNum.className = "pix-input pix-input-num"; core.brushSizeNum.value = core.brushSize;
        sizeWrap.append(core.brushSizeSlider, core.brushSizeNum);

        // Brush Hardness
        const hardLabel = document.createElement("div"); hardLabel.className = "pix-label"; hardLabel.innerText = "Hardness";
        const hardnessWrap = document.createElement("div"); hardnessWrap.className = "pix-row"; hardnessWrap.style.marginBottom = "10px";
        core.brushHardnessSlider = document.createElement("input"); core.brushHardnessSlider.type = "range"; core.brushHardnessSlider.min = "0"; core.brushHardnessSlider.max = "100"; core.brushHardnessSlider.value = Math.round(core.brushHardness * 100);
        core.brushHardnessNum = document.createElement("input"); core.brushHardnessNum.type = "number"; core.brushHardnessNum.className = "pix-input pix-input-num"; core.brushHardnessNum.value = Math.round(core.brushHardness * 100);
        hardnessWrap.append(core.brushHardnessSlider, core.brushHardnessNum);

        // Reset Eraser Mask
        const eraserDivider = document.createElement("div"); eraserDivider.className = "pix-eraser-divider";
        core.btnResetEraser = document.createElement("button"); core.btnResetEraser.className = "pix-btn"; core.btnResetEraser.innerText = "🔄 Reset Eraser Mask"; core.btnResetEraser.style.width = "100%"; core.btnResetEraser.style.marginTop = "2px"; core.btnResetEraser.style.opacity = "0.3"; core.btnResetEraser.disabled = true;
        ...
        core.removeBgBtn = document.createElement("button"); core.removeBgBtn.className = "pix-btn-accent"; core.removeBgBtn.style.opacity = "0.3"; core.removeBgBtn.style.pointerEvents = "none"; core.removeBgBtn.innerText = "✨ AI Remove Background";
