...
class PixaromaImageComposition:
    ...
    def load_composite(self, project_json):
        ...
        try:
            meta = json.loads(project_json)
            if not isinstance(meta, dict): return (empty_image, 1024, 1024)
            ...
            composite_path = meta.get("composite_path")
            if not composite_path: return (empty_image, doc_w, doc_h)
            ...
            if not full_path.startswith(input_dir + os.sep):
                print("[Pixaroma] Security: composite_path escapes input directory, blocked.")
                return (empty_image, doc_w, doc_h)

            if not os.path.exists(full_path): return (empty_image, doc_w, doc_h)
...
class _AnyType(str):
    """Wildcard type — accepted by any ComfyUI slot."""
    ...
    def show(self, source):
        try:
            ...
            if isinstance(source, torch.Tensor):
                text = (f"Tensor  shape={tuple(source.shape)}"
...
class PixaromaPaint:
    ...
    def load_painting(self, paint_json):
        ...
        try:
            ...
            if not full_path.startswith(input_dir + os.sep):
                print("[PixaromaPaint] Security: composite_path escapes input directory, blocked.")
                return (empty_image, doc_w, doc_h)
