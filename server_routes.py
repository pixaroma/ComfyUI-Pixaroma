...
_MODELS_ROOT = os.path.realpath(folder_paths.models_dir)
_KNOWN_TOP = ("models", "input", "output", "custom_nodes", "user", "temp")
# Reject absurdly long folder strings and control chars early.
_MAX_FOLDER_LEN = 512

print(f"[Pixaroma Note] COMFY_ROOT  = {_COMFY_ROOT}")
print(f"[Pixaroma Note] MODELS_ROOT = {_MODELS_ROOT}")
...
def _resolve_note_folder(folder: str):
    ...
    # Anchor to folder_paths.models_dir for anything that lives under models
    # (including bare "loras" → "<models_dir>/loras"). This is more reliable
    # than os.path.join(base_path, "models", …) because ComfyUI-Easy-Install
    # and similar distributions sometimes relocate models_dir outside the
    # ComfyUI base directory via extra_model_paths.yaml.
    if first == "models":
        rest = folder[len("models"):].lstrip("/")
        abspath = os.path.realpath(
            os.path.join(_MODELS_ROOT, rest) if rest else _MODELS_ROOT
        )
        # Security: must stay inside the configured models root.
        if abspath != _MODELS_ROOT and not abspath.startswith(_MODELS_ROOT + os.sep):
            return None, None
        display = "models" + (f"/{rest}" if rest else "")
        return abspath, display
    if first not in _KNOWN_TOP:
        # Bare "loras" / "checkpoints/foo" → under models
        abspath = os.path.realpath(os.path.join(_MODELS_ROOT, folder))
        if abspath != _MODELS_ROOT and not abspath.startswith(_MODELS_ROOT + os.sep):
            return None, None
        return abspath, f"models/{folder}"
    # input / output / custom_nodes / user / temp → under ComfyUI base
...
def _open_in_file_explorer(abspath: str) -> bool:
    ...
    On Windows, prefer launching `explorer.exe` directly over `os.startfile`.
    os.startfile invokes the shell's default "open" verb, which on some
    machines is rebound to a non-folder handler (e.g. an image previewer
    grabbing "folder" or the user's Desktop shell). `explorer.exe <path>`
    is unambiguous — it always opens a File Explorer window at <path>."""
    print(f"[Pixaroma Note] open folder → {abspath}")
    try:
        if sys.platform.startswith("win"):
            # Use the fully-qualified Explorer path and detach so we don't
            # block the aiohttp event loop on any stray output.
            subprocess.Popen(
                ["explorer", abspath],
                close_fds=True,
            )
...
async def note_check_folder(request):
    ...
    raw = data.get("folder", "")
    abspath, rel = _resolve_note_folder(raw)
    print(f"[Pixaroma Note] check_folder '{raw}' → {abspath} (exists={os.path.isdir(abspath) if abspath else False})")
    if abspath is None:
