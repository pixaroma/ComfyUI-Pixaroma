        ...
        return {
            "required": {
                ...
                "loop_safe": ("BOOLEAN", {"default": True,
                    "tooltip": "Ramp motion to zero across the first and last 0.5 s so the rendered clip loops with no visible jump. ON by default — typical use case (audio-reactive music videos / social loops) benefits, the 0.5 s fade is invisible on clips longer than ~5 s. Turn OFF for one-shot renders that won't loop, or for very short clips where you want full motion at the boundaries."}),
