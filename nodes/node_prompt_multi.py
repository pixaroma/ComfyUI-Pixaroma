...
class PixaromaPromptMulti:
    DESCRIPTION = (
        ...
        "workflow runs once for each enabled prompt and you get one image per "
        "prompt. Each prompt becomes its own item in the queue panel so you "
        "can cancel them individually.\n\n"
        "Click + Add row to add a prompt. Click the toggle pill to mute or "
        "unmute. Drag the handle on the left to reorder. If only one row is "
        "enabled, the node behaves like a normal prompt. If no rows are "
        "enabled, you will see a warning and the workflow will not run.\n\n"
