...
### **May 19, 2026 (1.3.47)**
- **NEW: Run Button FX, eight looks for the Run button.** Open Settings, search Pixaroma, and you will see a new "Run Button FX" option under "Run Button". Pick from None (default ComfyUI blue look), Pixaroma Orange (flat orange button matching the rest of the Pixaroma palette), Flash (orange lightning bolts shooting outward from the button when you click), Ignition (soft orange flame trail sweeping out to the left of the button on click), Thor (white and blue lightning bolts on click), Sparkle (continuous yellow sparkles around the button plus a bright sparkle burst when you press), Rocket (button shakes and a downward flame exhaust appears on click), and Shockwave (three concentric orange rings expand outward from the button on click). All effects are purely visual, never block running a workflow, and cost nothing when set to None.
- **NEW: Connection FX, magnetic feedback while wiring nodes together.** A second new setting under "Connections": when turned on, dragging a wire near any compatible input slot now shows a glowing orange dot at that slot, with a small stream of orange particles flowing back and forth between your cursor and the slot like a magnetic field. Release the wire on the slot and small orange lightning bolts burst out at the connection point. Default off, so the wiring experience stays unchanged unless you opt in. Costs nothing when off.
