...
### **May 17, 2026 (1.3.33)**
- **Remove Background Pixaroma: model dropdown built into the node, with HR support.** No more separate Load Background Removal Model node to wire in. Pick your model from a dropdown right on the node. Supports the standard model (good for clean objects), the high-resolution HR model (sharper masks on big images, more detail), and the HR matting model (soft alpha edges, beautiful for hair and fur). Drop the .safetensors files into ComfyUI/models/background_removal/; if the folder is empty the node tells you exactly what to download and where to put it. The right model resolution is picked automatically from the filename, so you don't have to think about it. Heads-up: workflows saved with the older version of the node lose the input cable on load, just pick your model from the new dropdown and re-save.
