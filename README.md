...
- **Run Log Pixaroma lets you label each run, so you can tell them apart.** A list of times tells you that something changed between runs, but not what. Now you can double-click any row and type a short note for that run: with style lora, seed 12345, base with no LLM, whatever helps. Press Enter to save, or just click away; Escape leaves it alone, and clearing the text removes the note. The note belongs to that run, so as newer runs push it down the list it travels with its own time, and it disappears with it when it drops off the bottom. Empty rows show a faint add note so it is easy to find, and your notes come along when you export the list or copy it. The rows themselves are tidier too: the LAST and BEST words gave up their space to make room for your text, since the newest run already has its orange bar and the fastest already has its lightning bolt. Nothing is lost from logs you already have, and the panel now ends neatly under the last run instead of leaving an empty strip below it.
- **Copying a Switch Pixaroma or Mute Switch Pixaroma now keeps everything you set up.** Copy and paste one (or duplicate it, or Alt-drag it) and it comes back exactly as it was: every row keeps the name you gave it, Switch stays on the row you had switched through, and Mute Switch keeps which branches are on and which are off. Before, only the first row's name came across, Switch could quietly end up sending a different model than the one you picked, and Mute Switch came back with every branch switched back on - so a copied setup could change what actually ran without showing any sign of it. Fixing it turned up a deeper cause that had been there a long while: every time a node was copied or a workflow was opened, these two nodes were quietly rebuilt with all 32 of their possible rows and then tidied straight back down again, which is why the damage stayed invisible for so long. That no longer happens at all, so they are lighter to load as well. Works in both the classic and the new node interface.
