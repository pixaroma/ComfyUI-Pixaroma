# Audio Studio Pixaroma Implementation Plan
...
**Goal:** Add a sibling node (Audio Studio Pixaroma) to Audio React Pixaroma. Same effect math; full live editor in a fullscreen browser overlay (WebGL preview + transport bar + tabbed sidebar). Both nodes share a registry-based effect engine. ~3500 LOC across multiple files.
...
| `CLAUDE.md` | MODIFY | +200 | Audio Studio section + new "do not regress" patterns |
| `README.md` | MODIFY | +30 | Audio Studio feature blurb |
...
This is the foundation for the upcoming Audio Studio Pixaroma sibling
...
## Milestone B — Audio Studio Python node + upload route

**Goal:** Land the Python side of Audio Studio: a new node `PixaromaAudioStudio` that takes optional image / audio inputs + a hidden `studio_json` config, resolves sources, and calls the engine. Plus the new upload server route.
...
Restart ComfyUI. In the Add-Node menu, navigate to `👑 Pixaroma`. Verify "Audio Studio Pixaroma" appears.
...
**Goal:** The goldens captured in A1 came from the un-refactored Audio React. After Milestone A, the engine produces (still byte-identical) frames. To make Audio Studio's own parity story crisp, regenerate the goldens through the explicit engine entry point and verify they match the A1 snapshot.
...
**Goal:** Register the extension, add an "Open Audio Studio" button to the node, wire up Pattern #9 persistence (`graphToPrompt` injection of `studio_json` from `node.properties.audioStudioState`).
...
app.registerExtension({
  ...
  async nodeCreated(node) {
    ...
    queueMicrotask(() => {
      // Nothing to render yet — node has only the "Open Audio Studio" button.
...
Restart ComfyUI. Drop the Audio Studio node. Verify:
- "Open Audio Studio" button appears.
...
js/audio_studio/index.js wires the 'Open Audio Studio' button on the node
...
Restart ComfyUI. Drop the node. Click "Open Audio Studio".
- ✅ Fullscreen overlay opens.
- ✅ Header shows × close + "Audio Studio Pixaroma" + Image/Audio pills + grayed-out SAVE button.
...
AudioStudioEditor.prototype._initRenderer = function () {
  ...
  if (!gl) {
    this.canvasHost.textContent = "WebGL2 unavailable — Audio Studio requires WebGL2. Use the basic Audio React node instead.";
    return;
  }
...
AudioStudioEditor.prototype._getMotionProgram = function (mode) {
  ...
  if (!src) {
    console.warn(`[Pixaroma] Audio Studio: motion mode ${mode} has no shader yet — using scale_pulse fallback`);
...
In ComfyUI, drop the Audio Studio node + Load Image upstream + connect. Click button to open.
...
In ComfyUI:
1. Drop: Load Image → Audio Studio Pixaroma ← Load Audio
2. Drop: Save Mp4 Pixaroma, wire Audio Studio's outputs (frames, audio, fps) to it.
3. Click "Open Audio Studio" on the node.
...
<html><head>
<meta charset="utf-8">
<title>Audio Studio — WebGL Parity Harness</title>
<style>
...
<h1>Audio Studio — WebGL Parity Harness</h1>
...
```
| Audio Studio Pixaroma — change effect math | `nodes/_audio_react_engine.py` (shared engine — math is here, not in nodes_audio_*.py) + sync update to `docs/audio-react-math.md` (single source of truth) + matching change to `js/audio_studio/shaders.mjs` (GLSL mirror) + browser parity harness (Task I1) re-run |
| Audio Studio Pixaroma — editor UI / sidebar | `js/audio_studio/ui.mjs` (controls / tabs) + `js/audio_studio/core.mjs` (open/close/save flow) |
| Audio Studio Pixaroma — transport / playback | `js/audio_studio/transport.mjs` |
| Audio Studio Pixaroma — WebGL pipeline | `js/audio_studio/render.mjs` (orchestration) + `js/audio_studio/shaders.mjs` (per-mode shaders) |
| Audio Studio Pixaroma — Python entry point | `nodes/node_audio_studio.py` (thin wrapper); engine math lives in `nodes/_audio_react_engine.py` |
| Audio Studio Pixaroma — upload route | `server_routes.py` `/pixaroma/api/audio_studio/upload` |
| Audio Studio Pixaroma — config schema | `nodes/node_audio_studio.py` `_migrate_cfg` + `js/audio_studio/index.js` `DEFAULT_CFG` (keep these in sync — Pattern #3 risk) |
```

- [ ] **Step 3: Add "Audio Studio Patterns (do not regress)" section**
...
These patterns were hard-won during Audio Studio v1 development. Regressing
...
```bash
git add CLAUDE.md
git commit -m "docs: CLAUDE.md updates for Audio Studio Pixaroma

Adds Frontend Directory Structure entry, Token-Saving Rules table rows,
and 'Audio Studio Patterns (do not regress)' section with 10 patterns
...
- **Same engine as Audio React** — the workflow renders identical frames
  ...
  scripted runs; use Audio Studio when you want to dial in the look
  interactively.
...
```bash
git add README.md
git commit -m "docs: README — Audio Studio Pixaroma feature section"
```
...
1. ComfyUI: drop Load Image + Load Audio + Audio Studio Pixaroma + Save Mp4 Pixaroma. Wire them.
2. Open Audio Studio. Tweak motion mode, intensity, an overlay or two.
3. Click Save.
4. Run workflow.
...
Print: `DONE: I4 (v1 acceptance criteria pass — Audio Studio Pixaroma is ready to ship)`
...
4. **Scope check** — single feature (Audio Studio + engine refactor + parity infra). Multi-session execution is built into the milestone structure.

5. **Cross-file consistency** — `DEFAULT_CFG` in `js/audio_studio/index.js` must mirror `Params` defaults in `_audio_react_engine.py`. Called out as Pattern #1 in the new "Audio Studio Patterns" CLAUDE.md section.
