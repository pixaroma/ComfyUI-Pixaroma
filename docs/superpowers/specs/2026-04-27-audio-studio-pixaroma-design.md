# Audio Studio Pixaroma — Design Spec
...
Audio Studio Pixaroma is a sibling node to the existing **Audio React Pixaroma**.
Same effect math, very different UX:

- **Audio React Pixaroma** — widgets-only, fast tweak-and-run scripted use (existing,
  unchanged).
- **Audio Studio Pixaroma (NEW)** — full live editor in a fullscreen browser overlay.
  Click "Open Audio Studio" on the node → WebGL preview canvas + transport bar +
...
widget surface; Audio Studio exposes the same effect set in a richer interactive
...
| `CLAUDE.md` | Adds Audio Studio entries to: Architecture / Frontend Directory Structure, Vue Frontend Compatibility (any new patterns discovered), Token-Saving Rules table, "do not regress" patterns section. |
| `README.md` | Adds Audio Studio Pixaroma feature blurb. |
...
class PixaromaAudioStudio:
    ...
    def generate(self, studio_json="{}", image=None, audio=None):
        ...
        if image is None: raise ValueError("[Pixaroma] Audio Studio — no image (no upstream wired and no inline image saved). Open the editor to load one.")
        if audio is None: raise ValueError("[Pixaroma] Audio Studio — no audio (no upstream wired and no inline audio saved). Open the editor to load one.")
        for diag in validate_params(params):
...
> "Audio Studio requires WebGL2, which is not available in this browser. Use
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ × Audio Studio Pixaroma   [Image: Upstream] [Audio: Inline]    [SAVE]   │  ← header (32px)
├──────────────────────────────────────────┬──────────────────────────────┤
...
- × close button (left)
- Title "Audio Studio Pixaroma" — orange (`#f66744`) accent
- Two upstream pills:
...
slots — there's no reason to expect Audio Studio to behave differently.

### Open-editor flow

1. User clicks **"Open Audio Studio"** button on the node.
...
- × Close (or Esc) with `state != saved_state` → modal:
  ...
  - Body: "You have unsaved changes to the Audio Studio. Discard them and close?"
...
- **`overlay_preset` widget** — deferred to its own follow-up spec; will apply
  to Audio React AND Audio Studio uniformly.
...
1. **Editor opens** from the node's "Open Audio Studio" button. Restores all
